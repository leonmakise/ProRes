...
class PairDataset(VisionDataset):
    ...
    def __init__(
        ...
        self.pairs = []
        self.weights = []
        #type_weight_list = [1, 3, 3, 3, 3]
        type_weight_list = [1,1,1]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        # exit()
        if 'dehaze' in pair_type:
            type_dict = torch.tensor([1, 0, 0]).unsqueeze(0)
        elif 'denoise' in pair_type:
            type_dict = torch.tensor([0, 1, 0]).unsqueeze(0)
        elif 'derain' in pair_type:
            type_dict = torch.tensor([0, 0, 1]).unsqueeze(0)
        else:
