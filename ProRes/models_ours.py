...
class UIP(nn.Module):
    ...
    def __init__(
        ...
        #self.decoder_norm = norm_layer(patch_size ** 2 * self.decoder_embed_dim)
    ...
    def forward_loss(self, pred, tgts):
        """
        tgts: [N, 3, H, W]
        pred: [N, 3, H, W]
        ...
        if self.loss_func == "l1l2":
            ...
            #loss = F.smooth_l1_loss(pred, target, reduction="none", beta=0.1)
    ...
    def forward(self, imgs, tgts):
        ...
        loss = self.forward_loss(pred, tgts)
        return loss, self.patchify(pred)
...
def uip_vit_large_patch16_input448x448_win_dec64_8glb_sl1(**kwargs):
    model = UIP(
...
if __name__ == '__main__':
    ...
    tgt = torch.rand(2,3,448,448)
    # valid = torch.ones(2,3,448,448)

    # bool_masked_pos = torch.zeros(2,int(448/16),int(448/16))
