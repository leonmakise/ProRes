...
def train_one_epoch(model: torch.nn.Module,
    ...
    wandb_images = []
    for data_iter_step, (samples,targets, bool_masked_pos, valid, flag, image_ori) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            loss, y = model(samples, targets)
            #loss, y = model(samples, targets, prompt)
...
def evaluate_pt(data_loader, model, device, epoch=None, global_rank=None, args=None):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            #bool_masked_pos[:, :bool_masked_pos.shape[1]//2] = 0
            #bool_masked_pos[:, bool_masked_pos.shape[1]//2:] = 1
            loss, y = model(samples, targets)
            #loss, y = model(samples, targets, prompt)
