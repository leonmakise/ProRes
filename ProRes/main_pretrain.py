...
from engine_pretrain import train_one_epoch, evaluate_pt

from data.unif import PairDataset
#from data.unif_val import PairDataset_val
#from data.data_multi import PairDataset
...
def get_args_parser():
    ...
    parser.add_argument('--min_random_scale', default=0.2, type=float,
                        help='Minimal random scale for randomresizecrop (default: 0.2)')
    ...
    parser.add_argument('--half_mask_ratio', default=0., type=float,
                        help='ratio of using half mask during training (default: 0.)')
    ...
    parser.set_defaults(use_two_pairs=False)
...
def main(args, ds_init):
    ...
    # define the model
    model = models_ours.__dict__[args.model]()
    ...
    dataset_train = PairDataset(args.data_path, args.json_path, transform=transform_train, transform2=transform_train2, transform3=transform_train3, masked_position_generator=masked_position_generator, use_two_pairs=args.use_two_pairs, half_mask_ratio=args.half_mask_ratio)
    ...
    if global_rank == 0 and args.log_wandb:
        ...
        if args.resume == '':
            wandb.init(project="UIP", name=experiment, config=args)
        else:
            wandb.init(project="UIP", name=experiment, config=args, resume=True)
