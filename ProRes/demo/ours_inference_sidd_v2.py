...
if __name__ == '__main__':
    ...
    prompt_type = 'groundtruth' if args.pred_gt else 'gt_sub_input'

    ckpt_file = 'checkpoint-{}.pth'.format(epoch)
    assert ckpt_dir[-1] != "/"
    dst_dir = os.path.join('models_inference', ckpt_dir.split('/')[-1],
                           "sidd_inference_epoch{}_{}".format(epoch, os.path.basename(prompt).split(".")[0]))
    ...
    for img_path in tqdm.tqdm(img_path_list):
        ...
        prompt_org = np.load('datasets/low_level/ssid.npy')
