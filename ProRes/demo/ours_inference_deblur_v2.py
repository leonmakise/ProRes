...
if __name__ == '__main__':
    ...
    prompt_type = 'groundtruth' if args.pred_gt else 'target_sub_input'

    ckpt_file = 'checkpoint-{}.pth'.format(epoch)
    assert ckpt_dir[-1] != "/"
    dst_dir = os.path.join('models_inference', ckpt_dir.split('/')[-1],
                           "deblur_inference_epoch{}_{}".format(epoch, os.path.basename(prompt).split(".")[0]))
    ...
    model_mae.eval()
    datasets = ['GoPro','HIDE','RealBlur_R','RealBlur_J']

    psnr_alldatasets = []
    ssim_alldatasets = []
    print(datasets)
    img_src_dir = "datasets/low_level/deblur/test/"
    for dset in datasets:
        ...
        for img_path in tqdm.tqdm(img_path_list):
            ...
            prompt_org = np.load('datasets/low_level/deblur.npy')
            ...
            # load gt
            rgb_gt = Image.open(img_path.replace('input', 'groundtruth')).convert("RGB")  # irrelevant to prompt-type
            ...
            psnr = calculate_psnr(rgb_restored*255., rgb_gt*255., 0, test_y_channel=False)
